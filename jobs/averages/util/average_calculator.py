...
def last_n_rows(df, partition_col_1, partition_col_2, num_of_rows):
    window = Window.partitionBy(partition_col_1, partition_col_2).orderBy(
        fn.desc("date")
    )
...
def calculate_partitioned_avg_single(partition_col, target_col):
    window = Window.partitionBy(partition_col).orderBy(fn.col("date").desc())
    return fn.round(fn.avg(target_col).over(window.rowsBetween(0, 4)), _decimal_points)
...
def last_value_in_col(df, partition_col, target_col, new_col):
    ...
    last_value_df = df.withColumn(
        new_col, fn.last(target_col, ignorenulls=True).over(window)
    )
